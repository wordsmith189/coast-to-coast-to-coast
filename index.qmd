...
date: Sep-2025
date-modified: Nov-2025
author:
 - name: Stefan Dollinger
   affiliation: The University of British Columbia
   url: https://english.ubc.ca/profile/stefan-dollinger/
 - name: Lars Hinrichs
...
To appear in James Walker, Debbie Loakes and Ksenia Gnevsheva (eds.), 2026. *Perceptions and attitudes in multilingual contexts*. Special issue, *Australian Journal of Linguistics*. 
:::
...
#| fig-cap: "Density plot illustrating the distribution of age and gender among those whose formative years were spent in Canada."
#| echo: false
...
ggplot(df_counts, aes(x = age, y = n, colour = gender)) +
  ...
  labs(
    x     = "Age",
    y     = "N respondents",
    title = "Respondents count by age and gender"
  ) +
...
ggsave("fig/fig-1.png", width = 8, height = 5, dpi = 600)
```

:::{.callout-tip appearance="simple"}
Download this figure [here](fig/fig-1.png).
:::
...
#| fig-cap: "Likert-scale responses to questions on the linguistic autonomy of CanE among the Canadian residents (2,723 ≤ *N* ≤ 2,813)."
#| eval: true
#| echo: false
#| fig-width: 9
#| fig-height: 3.5
...
p <- df_long_summary %>%
  ...
  geom_col(width = 0.8, , color="grey40") +
  
  # ----------------------------------------------------
  # 4a. Percentage labels (ON TOP of the bars)
  # Applies to all questions
  geom_text(aes(label = Percent_Label), 
            vjust = -0.5, 
            size = 3.5) +
  
  # 4b. Text labels inside the bars for the Q6 (software) plot
  geom_text(data = . %>% filter(Question_Short == "software"), # Filter data for only Q6
            aes(label = Response, y = Percentage / 2), # Place label in the middle of the bar
            color = "white",
            fontface = "bold",
            size = 3.5) +
  # ----------------------------------------------------
  ...
  scale_fill_manual(
    ...
    # --- CHANGE 1: RESTRICT LEGEND TO 5 LIKERT VALUES ---
  ...
  # Theme and Labels
  labs(
    title = "Standardized Survey Responses to Language and Identity Questions",
    subtitle = "Percentage distribution of responses, NA values excluded per question.",
    x = NULL,
    y = NULL, 
    fill = NULL 
  ) +
...
:::{.aside}
Since we are able to use the caption for explanation, should we perhaps leave out the title within the figure?
:::
...
ggsave("fig/fig-2.png", width = 9.5, height = 4.1, dpi = 600)
```

:::{.callout-tip appearance="simple"}
Download this figure [here](fig/fig-2.png).
:::
...
:::{.aside}
I don't have the data to reproduce the two panels of Figure 3; can you send them to me? 
:::
...
#| fig-cap: "Have you heard of either \"Standard Canadian English\" or \"Canadian Standard English\"?"
#| eval: true
#| echo: false
#| fig-width: 9
#| fig-height: 7
...
p3 <- ggplot(df_percent_labeled, aes(x = st_heard, y = percent, fill = st_heard)) +
  geom_bar(stat = "identity", width = .55, color="grey40") +
...
:::{.aside}
By the way - is it advised that we use a grey scale instead of colors in the plots? 
:::
...
ggsave("fig/fig-4.png", width = 9, height = 7, dpi = 600)
```

:::{.callout-tip appearance="simple"}
Download this figure [here](fig/fig-4.png).
:::
...
#| fig-cap: "Responses to \"Have you heard of StCanE?\" by linguistic background."
#| eval: true
#| echo: false
#| fig-width: 6
#| fig-height: 3.5
...
df_plot_data %>%
  ...
  geom_col(width = 0.5, position = "dodge", color="grey40") +
  
  geom_text(
    aes(label = N_Label),
    position = position_dodge(width = 0.5), # Must match geom_col's width
  ...
  labs(fill = "Linguistic Background",
       caption = "NB: bar size corresponds to %."
       y = NULL,
       x = NULL) +
...
ggsave("fig/fig-5.png", width = 6, height = 3.5, dpi = 600)
```

:::{.callout-tip appearance="simple"}
Download this figure [here](fig/fig-5.png).
:::
...
:::{.aside}
**NB the table numbering** is off in the version of the ms. that I got. The correct number for this table is 3.
:::
...
:::{.aside}
*introduce [...] into the mix* >> add [...] into the mix
:::
